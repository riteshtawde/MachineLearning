...
weight_func = function(val){
  return (1/val)
}

knn_dist_weight_algo = function(dt_set,
                                index,
                                train,
                                test,
                                train_labels,
  ...
  for (k in (near_neigh - 2):(near_neigh + 2)) {
    ...
    for (i in 1:nrow(test)) {
      ...
      df_dist[,1] = 1/df_dist[,1]
      agg = aggregate(df_dist[,1], by=list(df_dist[,2]), FUN=sum)
      predictions[i] = as.character(agg[which.max(agg[,2]),1])
    }
...
plot_graphs_dist = function(title, filename, xvalues, yvalues) {
  title_var = paste(title)
  png(filename = paste("plots/knn/knn_dist_weight/", filename, ".png", sep = ""))
  plot(
    x = xvalues,
    y = yvalues,
...
cat("############################ KNN USing Weighted distance ############################\n")
start_time = Sys.time()
